...
for ip in $MASTER_IP $SLAVES_IP; do
    scp $SSH_OPTS $ARCHIVE_TARBALL centos@$ip:~/
    arg="slave $ip"
    [[ $master -eq 1 ]] && arg="master" && master=0
    ssh $SSH_OPTS centos@$ip "cd ~/; tar -xzf $ARCHIVE_TARBALL"
    ssh $SSH_OPTS centos@$ip "bash ~/install-scripts/install-spark.sh $arg"
    shift
done
